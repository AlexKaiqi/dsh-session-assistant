...
export const Config: z<SessionAssistantSettings> = z.object({
  recognitionProvider: z.union(['browser', 'openai-realtime', 'doubao-realtime', 'composed'] as const).description('语音后端').default(DEFAULT_SETTINGS.recognitionProvider),
  ...
  composedLanguageSource: z.union(['current-session', 'fixed'] as const).description('组合语音管线语言模型来源').default('current-session'),
