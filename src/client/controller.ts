...
export class VoiceController {
  ...
  async stop(): Promise<void> {
    ...
    if (!this.disposed) this.publish({ status: 'idle', phase: 'idle', transcript: '', error: undefined })
  }
  ...
  async consume(event: VoiceEvent): Promise<void> {
    ...
    if (event.type === 'status') this.publish({ status: event.connected === false || event.status === 'closed' ? 'idle' : 'active' })
      ...
      if (dictation && event.final && (event.role ?? event.source ?? 'input') === 'input') this.appendDictation(event.text)
    } else if (event.type === 'interrupted') this.publish({ phase: 'listening' })
    else if (event.type === 'error') this.publish({ status: 'error', error: event.message })
  ...
  private async applyTool(event: ToolEvent): Promise<void> {
    if (this.disposed || this.handle === undefined) return
