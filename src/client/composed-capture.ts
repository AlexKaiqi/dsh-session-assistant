...
export interface CaptureRecognitionPort {
  recognize(options: {
    ...
    onTranscript(event: { text: string; final: boolean }): void
...
export function captureComposedUtterance(input: {
  ...
  return new Promise((resolve, reject) => {
    let settled = false
    let handle: CaptureRecognitionHandle
    const finish = (error?: unknown, audio?: CapturedPcmAudio) => {
      if (settled) return
      settled = true
      input.signal.removeEventListener('abort', aborted)
      handle?.close()
      if (error !== undefined) reject(error)
      else if (audio === undefined) reject(new Error('Browser capture returned no audio'))
      else resolve(audio)
    }
    ...
    handle = input.voiceAgent.recognize({
      ...
      onTranscript(event) {
        ...
        if (event.final) finish(undefined, handle.takeAudio?.())
      },
...
export function playAudioUri(uri: string, signal: AbortSignal, createAudio: (uri: string) => HTMLAudioElement = value => new Audio(value)): Promise<void> {
  return new Promise((resolve, reject) => {
