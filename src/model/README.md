...
The browser executes these UI-local operations. No filesystem, shell, Agent
tool, or credential is exposed to the voice model. Realtime provider
credentials are resolved by `dsh-multi-model-provider`; provider adapters never
expose them to the browser model.
