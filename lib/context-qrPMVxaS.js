...
function buildBoundedContext(session, draft, mode) {
	if (mode === "off") return "";
...
function nodeEntries(session) {
	...
	if (!nodes) return [];
	const entries = [];
	for (const value of nodes.values()) entries.push(value);
	return entries;
}
function questionText(argumentsRaw) {
	if (typeof argumentsRaw !== "string") return "";
	try {
		const questions = JSON.parse(argumentsRaw)?.questions;
		if (!Array.isArray(questions)) return "";
		return questions.map((entry) => {
			const question = entry;
			return `${typeof question.question === "string" ? question.question : ""}${Array.isArray(question.options) && question.options.length ? `（选项：${question.options.map((option) => option?.label).filter((label) => typeof label === "string").join(" / ")}）` : ""}`.trim();
		}).filter(Boolean).join(" ");
	} catch {
		return "";
	}
...
function questionsInSession(session) {
	const questions = [];
...
//#endregion
export { countAssistantSteps as n, questionsInSession as r, buildBoundedContext as t };
