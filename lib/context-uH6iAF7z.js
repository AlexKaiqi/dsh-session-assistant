...
function assistantReplyCursor(session) {
	let cursor;
	for (const node of nodeEntries(session)) {
...
*/
function finalAgentReplyAfter(session, cursor) {
	if (session.running === true) return void 0;
	let afterCursor = cursor === void 0;
	let reply;
	for (const node of nodeEntries(session)) {
		if (!afterCursor) {
...
//#endregion
export { finalAgentReplyAfter as a, OPENAI_REALTIME_VOICES as c, countAssistantSteps as i, normalizeSettings as l, awarenessEventsInSession as n, DECLARED_SETTINGS_FIELDS as o, buildBoundedContext as r, DEFAULT_SETTINGS as s, assistantReplyCursor as t };
