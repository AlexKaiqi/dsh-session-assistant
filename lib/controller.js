import { n as countAssistantSteps, r as questionsInSession } from "./context-qrPMVxaS.js";
...
var VoiceController = class {
	...
	*/
	async executeTool(name, args, control) {
		...
		if (draft === void 0 || draft.length > 24e3 || emptySubmit || !validUpdate) {
			this.publish({
				status: "error",
				errorCode: "empty_submit"
			});
			control.resolve({
				ok: false,
				error: emptySubmit ? "There is no draft content to submit. Ask the user to dictate what to send first." : "Invalid draft tool arguments."
			});
			return;
		}
		const current = this.deps.getInput().draft;
...
function voiceConversationOptions(settings, context) {
...
function voiceAgentPreviewOptions(settings) {
	const openai = settings.recognitionProvider === "openai-realtime";
	return {
...
//#endregion
export { VoiceController, saLog, voiceAgentPreviewOptions, voiceConversationOptions };
