import { a as finalAgentReplyAfter, i as countAssistantSteps, l as normalizeSettings, n as awarenessEventsInSession, o as DECLARED_SETTINGS_FIELDS, t as assistantReplyCursor } from "./context-uH6iAF7z.js";
...
var VoiceController = class {
	...
	*/
	...
	submitRecognizedText(text, signal) {
		...
		if (!transcript) return Promise.reject(/* @__PURE__ */ new Error("Recognized text is empty"));
		if (this.disposed) return Promise.reject(/* @__PURE__ */ new Error("Session Assistant is disposed"));
		...
		this.replyCursor = session ? assistantReplyCursor(session) : void 0;
		return new Promise((resolve, reject) => {
			...
			const aborted = () => {
				if (this.finalReplyWaiter?.resolve === done) this.finalReplyWaiter = void 0;
				fail(signal.reason ?? new DOMException("Aborted", "AbortError"));
			};
			this.finalReplyWaiter = {
				resolve: done,
				reject: fail
			};
...
function voiceAgentPreviewOptions(settings, routeId) {
	const openai = settings.recognitionProvider === "openai-realtime";
	const previewText = settings.recognitionLang === "zh-CN" ? "请以 Session Assistant 产品向导的身份主动介绍你是谁、你的定位、三到四项核心能力、与主 Agent 的分工和能力边界，然后邀请我继续询问。" : "As the Session Assistant Product Guide, proactively introduce who you are, your positioning, three or four core capabilities, how you work with the primary Agent, and your boundaries, then invite my questions.";
	return {
