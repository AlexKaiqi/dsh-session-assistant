...
import { captureComposedUtterance, playAudioUri } from "./composed-capture.js";
import { ComposedVoiceTurn, composedPipeline } from "../voice-pipeline.js";
...
function MicControl(props) {
    ...
    const [composedPhase, setComposedPhase] = React.useState('idle');
    const composedAbort = React.useRef();
    const composedTurn = React.useRef();
    React.useEffect(() => () => {
    ...
    const runComposed = async () => {
        if (!props.voiceAgent || !props.composedVoice)
            throw new Error('Composed voice services are unavailable');
        if (!settings.composedAsrRoute.trim() || !settings.composedTtsRoute.trim())
            throw new Error('Explicit ASR and TTS routes are required');
        if (settings.composedLanguageSource !== 'current-session')
            throw new Error('Fixed composed language models are not enabled');
        ...
        try {
            ...
            const turn = new ComposedVoiceTurn(pipeline, {
                ...
                synthesize: async (text, stage, signal) => {
                    ...
                    if (!result.ok || !result.value)
                        throw new Error(result.error?.message || 'Speech synthesis failed');
            ...
            if (composedAbort.current === outer)
                composedAbort.current = undefined;
    ...
    const onPointerDown = () => {
        if (composed || !controller.canEnterStandby)
            return;
        longPressed.current = false;
        cancelPress();
...
export async function apply(ctx) {
    ...
    const composedVoice = ctx.get('remote.composedVoice');
