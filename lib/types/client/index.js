...
import { VoiceController, assistantSettingsContext, matchesWakePhrase, selectVoiceRoute, voiceConversationOptions, voiceAgentPreviewOptions, saLog } from "./controller.js";
import { buildBoundedContext } from "./context.js";
import { dictionaries, NS, phaseLabel } from "./locales.js";
export const inject = ['slots', 'locale', 'remote', 'voiceAgent'];
function browserRecognitionSession(voiceAgent, language, ownerId, t) {
    let listener;
    let closed = false;
    const pending = [];
    const emit = (event) => {
...
function useController(props) {
    ...
    const metadataRef = React.useRef({ sessionId: props.sessionId });
    ...
    return React.useMemo(() => props.controllerFor(props.sessionId, props.inputActions, inputRef, sessionRef, metadataRef), [props.sessionId, props.inputActions, props.controllerFor]);
}
...
function planNoticeText(t, event) {
    if (event.phase === 'completed')
        return `${t('agentPlanCompleted')} ${event.completed}/${event.total}`;
    if (event.active.length)
        return `${t('agentPlanProgress')} ${event.completed}/${event.total} · ${event.active.join('；')}`;
...
function VoiceAgentPreview(props) {
    ...
    const release = React.useCallback((close) => {
        ...
        if (close && current)
            void Promise.resolve(current.end());
    }, []);
        ...
        try {
            ...
            const dispose = conversation.subscribe(event => {
                ...
                if (event.type === 'transcript')
                    ...
                    setStatus(spoke.current ? 'done' : 'idle');
                    setSpeaking(false);
                }
            });
            if (generation.current !== current || handle.current !== conversation) {
                dispose();
                return;
            }
    ...
    return React.createElement('div', { className: 'sa-preview' }, [
        React.createElement('div', { key: 'toolbar', className: 'sa-preview-toolbar' }, [
        ...
        transcript ? React.createElement('div', { key: 'text', className: 'sa-preview-transcript', 'aria-live': 'polite' }, transcript) : null,
    ]);
}
function SettingsSection(props) {
    ...
    const field = (key, value) => { setSaved(false); setDraft(current => ({ ...current, [key]: value })); };
    ...
    return React.createElement('div', { className: 'sa-settings' }, [
        ...
        draft.recognitionProvider !== 'browser' && draft.recognitionProvider !== 'composed' ? row('model', props.t('realtimeModel'), React.createElement('select', { value: draft.recognitionProvider === 'openai-realtime' ? draft.openaiRealtimeModel : draft.doubaoRealtimeModel, onChange: (event) => field(draft.recognitionProvider === 'openai-realtime' ? 'openaiRealtimeModel' : 'doubaoRealtimeModel', event.target.value) }, [React.createElement('option', { key: 'auto', value: '' }, props.t('autoSelect')), ...models.map(model => React.createElement('option', { key: model.id, value: model.id }, model.displayName || model.id))])) : null,
        draft.recognitionProvider === 'composed' ? row('composedAsr', 'ASR 路由（显式）', React.createElement('input', { value: draft.composedAsrRoute, placeholder: 'doubao-agent-plan/seed-asr-2.0-stream', onChange: (event) => field('composedAsrRoute', event.target.value) })) : null,
        draft.recognitionProvider === 'composed' ? row('composedTts', 'TTS 路由（显式）', React.createElement('input', { value: draft.composedTtsRoute, placeholder: 'doubao-agent-plan/seed-tts-2.0-http', onChange: (event) => field('composedTtsRoute', event.target.value) })) : null,
        draft.recognitionProvider === 'composed' ? row('composedLanguage', '语言模型', React.createElement('select', { value: draft.composedLanguageSource, onChange: (event) => field('composedLanguageSource', event.target.value) }, [React.createElement('option', { key: 'current', value: 'current-session' }, '使用当前 Session 模型'), React.createElement('option', { key: 'fixed', value: 'fixed' }, '固定指定模型')])) : null,
        draft.recognitionProvider === 'composed' && draft.composedLanguageSource === 'fixed' ? row('composedProvider', '固定 Provider', React.createElement('input', { value: draft.composedLanguageProvider, placeholder: 'volcengine-agent-plan', onChange: (event) => field('composedLanguageProvider', event.target.value) })) : null,
        draft.recognitionProvider === 'composed' && draft.composedLanguageSource === 'fixed' ? row('composedModel', '固定模型', React.createElement('input', { value: draft.composedLanguageModel, placeholder: 'glm-5-2-260617', onChange: (event) => field('composedLanguageModel', event.target.value) })) : null,
...
.sa-settings{display:flex;flex-direction:column;gap:14px;padding:4px 0 20px}.sa-setting-row{display:grid;grid-template-columns:minmax(140px,220px) minmax(220px,1fr);gap:20px;align-items:center}.sa-setting-label{color:var(--dsw-alias-label-primary)}.sa-setting-control{min-width:0}.sa-setting-control>select,.sa-setting-control>input:not([type=checkbox]){box-sizing:border-box;width:100%;min-height:32px}.sa-setting-control>input[type=checkbox]{display:block;margin:0}.sa-preview{display:flex;min-width:0;flex-direction:column;align-items:flex-start;gap:8px;min-height:32px}.sa-preview-toolbar{display:flex;min-width:0;align-items:center;gap:10px}.sa-preview button{flex:none}.sa-preview-transcript{box-sizing:border-box;width:100%;max-height:112px;overflow-y:auto;padding:8px 10px;border-left:2px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xs-13);line-height:1.55;white-space:pre-wrap;word-break:break-word}.sa-preview-playing{color:var(--dsw-alias-state-info-primary)}.sa-preview-status{min-width:0;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xs-13)}.sa-wave{display:inline-flex;align-items:flex-end;gap:2px;height:20px;color:var(--dsw-alias-state-info-primary)}.sa-wave i{width:3px;height:35%;border-radius:1px;background:currentColor;animation:sa-wave-listen 1.4s ease-in-out infinite}.sa-wave i:nth-child(2n){animation-delay:.18s}.sa-wave i:nth-child(3n){animation-delay:.36s}.sa-wave-speak i{animation-name:sa-wave-speak;animation-duration:.45s}@keyframes sa-wave-listen{0%,100%{height:30%}50%{height:55%}}@keyframes sa-wave-speak{0%,100%{height:15%}50%{height:95%}}.sa-settings-actions{display:flex;flex-direction:column;align-items:flex-start;gap:8px;margin-left:240px}@media(max-width:700px){.sa-setting-row{grid-template-columns:1fr;gap:6px}.sa-settings-actions{margin-left:0}.sa-setting-row+.sa-setting-row{margin-top:4px}}
`;
export async function apply(ctx) {
    ...
    const voiceAgent = ctx.voiceAgent;
    ...
    let models = [];
    try {
        models = await voiceAgent.models();
    }
    ...
    let standbyHandle;
    const controllerFor = (sessionId, inputActions, input, session, metadata) => {
        ...
        if (!controller) {
            controller = new VoiceController({
                ...
                startConversation: async (initialUserText, initialAudio) => {
                    ...
                    catch { /* Host protocol fallback remains available. */ }
                ...
                standby: {
                    enter() {
                        ...
                        if (!wake || !voiceAgent.capabilities().recognition)
                            return false;
                        try {
