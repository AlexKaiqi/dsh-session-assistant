    ...
    const controllerFor = (sessionId, inputActions, input, session) => {
        ...
        if (!controller) {
            controller = new VoiceController({
                ...
                startConversation: async (initialUserText) => {
                    ...
                    return settings().recognitionProvider === 'browser'
                        ...
                        : voiceAgent.startConversation({ ...options, ownerId: `session-assistant:${sessionId}`, initialUserText });
                },
                standby: {
                    enter() {
                        if (standbyHandle)
                            ...
                            standbyHandle = voiceAgent.recognize({
                                ...
                                onTranscript: event => {
                                    ...
                                    if (matchesWakePhrase(event.text || '', wake)) {
                                        ...
                                        void controller.start((event.text || '').trim().slice(0, 20_000));
                                    }
                                },
                                onError: () => { },
                            });
