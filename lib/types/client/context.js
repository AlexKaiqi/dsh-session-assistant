...
export function assistantReplyCursor(session) {
    let cursor;
    for (const node of nodeEntries(session)) {
        if (node.kind !== 'assistant-step' || node.visibility === 'hidden' || node.data?.status === 'running')
            continue;
        cursor = node.id;
    }
    return cursor;
 ...
 */
export function finalAgentReplyAfter(session, cursor) {
    ...
    for (const node of nodeEntries(session)) {
        ...
        if (status !== 'settled' && status !== 'interrupted')
            continue;
