...
function realtimePreviewInstructions(sample = "") {
	return ["You are a voice preview. Speak exactly one short sample sentence, then stop. Do not ask questions, call tools, mention settings, or continue the conversation.", sample ? `Exact sample sentence:\n${sample}` : "Say a brief friendly greeting."].join("\n\n");
}
...
function previewProfile({ id = "session-assistant-preview", openaiVoice } = {}) {
	return {
		id,
		instructions: realtimePreviewInstructions,
		tools: [],
		voice: openaiVoice ? { openai: openaiVoice } : {}
	};
}
