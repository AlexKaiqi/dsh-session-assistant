...
- **独立统一产品介绍与语音导览**：新增可直接供人阅读的 `INTRODUCTION.md`，正式助手和设置页导览运行时读取同一文件，避免三份描述漂移；原“助手音色试听”正式更名为“语音导览”，使用实际所选 Realtime 音色回答核心能力、可选集成、边界与推荐工作流，并明确区分介绍模式与可执行的主 Agent
- **语音插话即打断**：助手播报时，用户直接开口会由共享 Realtime Voice 在浏览器音频层立即停止当前播放、取消旧回复并继续传输本次插话；不再等待助手说完，也不再显示需要手动点击的“打断”按钮
