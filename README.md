...
OpenAI／豆包路由、Profile、Realtime 音色、识别语言、上下文与唤醒词是唯一保留的语音设置。助手音色试听会以所选 Realtime 音色先打招呼，再保持麦克风开启供你直接对话。Provider、模型、Realtime 音色与识别语言在下次连接时生效，唤醒词在下次长按麦克风进入待机时生效。插件不再注册浏览器/系统朗读按钮、自动朗读、朗读音色或朗读语速。
