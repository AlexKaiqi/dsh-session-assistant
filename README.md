...
| # | 功能 | 说明 |
|---|---|---|
| 1 | 🎤 语音输入 | 输入框旁麦克风按钮：点一下开始说话，**实时中间结果上屏**，再点一下停止，文字进输入框；**聆听中可直接打字修改**（识别继续，停止时自动补上后续识别文本） |
...
1. **语音输入**：点输入框工具条上的 🎤 → 浏览器弹麦克风授权（允许）→ 说话（中间结果实时上屏）→ 再点 🎤 停止 → 文字已进输入框，回车发送；识别中**随时可以直接打字改错字**——气泡会提示「正在打字修改，识别继续…」，停止时后续识别文本自动补在框尾，不会覆盖你的修改
