...
test('model contract keeps the voice role bounded and exposes exact tool/result surfaces', async () => {
  ...
  assert.match(mod.PROMPT, /不执行工程或外部任务/)
  ...
  assert.match(humanIntroduction, /我是 \*\*Session Assistant（会话助手）\*\*/)
  assert.match(humanIntroduction, /语音讨论与澄清/)
  assert.match(guidePrompt, /identity is Session Assistant Product Guide/)
  assert.match(guidePrompt, /never say you lack information about your identity or capabilities/)
