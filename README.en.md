...
OpenAI/Doubao route, profile, Realtime voice, recognition language, context, and wake word are the only voice settings retained. The assistant-voice preview greets you with the selected Realtime voice and then keeps the microphone open for a live conversation. Provider/model/Realtime voice/language changes apply on the next connection, and wake-word changes on the next standby entry. The plugin no longer registers browser/system read-aloud buttons, automatic read-aloud, read-aloud voice, or read-aloud speed.
