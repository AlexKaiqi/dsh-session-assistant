...
The voice model can only call `update_working_draft`, `prepare_agent_handoff`, `submit_to_agent`, `end_voice_session`, or `organize_notes`. Requests that need workspace contents, current state, tools, side effects, or verification first become a visible handoff awaiting confirmation; they are never executed or submitted implicitly. This plugin registers the executors with the runtime while keeping the product boundaries: draft changes use `inputActions.setDraft(fullText)`, submission uses `inputActions.submit()`, and curation is delegated to the curator agent. Each controller remains bound to the Slot `sessionId`; disposed or closed controllers reject late tool application.
