...
- `dsh-realtime-voice` owns browser/provider media transports, the tool-execution loop (tool registry, async results, dual output), audio-input arbitration, and exposes the provider-neutral `realtimeVoice` Client service.
- Each Session claims the exclusive audio-input lease as `session-assistant:<sessionId>` and registers tool executors with `realtimeVoice.registerTools` under the same owner prefix; the runtime executes and settles tool events (settle the result first so the model keeps speaking, then run the submission and context refresh). If the global Pet Assistant or another voice product owns the lease, startup fails explicitly instead of capturing in parallel.
